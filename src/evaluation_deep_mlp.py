...
def classification_metrics(cm, classes=[-1, 0, 1]):
    precision, recall, f1 = {}, {}, {}
...
def main():
    ...
    X_test_norm = (X_test - mu_deep) / sigma_deep
    preds = predict_deep_mlp(X_test_norm, params_deep)
    ...
    classes = [-1, 0, 1]
    print(cm)
    precision, recall, f1 = classification_metrics(cm, classes=classes)
    print("\nClassification Metrics:")
    print("Class\tPrec\tRecall\tF1")
    for cls in classes:
        print(f"{cls}\t{precision[cls]:.2f}\t{recall[cls]:.2f}\t{f1[cls]:.2f}")
